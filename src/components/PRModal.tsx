...
export default function PRModal({
  ...
  const [formData, setFormData] = useState({
    title: "",
    description: "This PR was generated by DevElevator.",
    branch: "",
  });
  ...
  // Initialize Modal (Check Token -> Fetch Content -> Setup Form)
  useEffect(() => {
    if (!isOpen) return;
    ...
    const initModal = async () => {
      ...
      try {
        // 1. Verify GitHub Token exists in Vault
        const tokenRes = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/users/integrations`,
          { withCredentials: true },
        );

        if (tokenRes.data.hasGithubToken) {
          setHasToken(true);

          // 2. Setup standard PR state
          setTargetPath(filePath);
          setFormData({
            title: `Fix: Update ${filePath.split("/").pop()}`,
            description: "This PR was generated by DevElevator.",
            branch: `develevator-fix-${Date.now()}`,
          });
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div
        ...
        {/* -------------------------------------------------- */}
        {/* MODAL BODY (Conditional Rendering Based on Token)  */}
        {/* -------------------------------------------------- */}
